...
const { syncApplications } = require('../services/applicationService');
...
async function run() {
  try {
    ...
    console.log(
      `[syncApplicationsJob] Success fetched=${result?.fetched} eligible=${result?.eligible} inserted=${result?.inserted} skipped=${result?.skipped}`
    );
  } catch (error) {
    console.error('[syncApplicationsJob] Failed:', error.message);
    process.exitCode = 1;
  } finally {
